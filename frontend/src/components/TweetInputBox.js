...
const TweetInputBox = ({ onNewTweet }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md px-6 py-5 mb-4">
      <div className="flex items-start space-x-4">
        {user && (
          <div className="w-12 h-12 rounded-full bg-blue-400 flex items-center justify-center text-white text-2xl font-bold flex-shrink-0 border-2 border-white">
        ...
        <form onSubmit={handleSubmit} className="flex-grow relative">
          <div className="relative">
            <textarea
              className={`w-full p-4 text-lg text-gray-900 dark:text-white bg-transparent border-none focus:ring-0 focus:outline-none resize-none peer ${tweetContent ? 'pt-6' : ''}`}
              rows="3"
              value={tweetContent}
            ...
            <label 
              ...
              className={`absolute left-4 transition-all duration-200 ease-in-out 
                ${tweetContent 
                  ? 'top-2 text-base text-gray-500 dark:text-gray-400' 
                  : 'top-1/2 -translate-y-1/2 text-lg text-gray-500 dark:text-gray-400'
                } peer-focus:top-2 peer-focus:text-base peer-focus:text-blue-500`}
            >
              What's happening?
            </label>
          </div>
          ...
          <div className="mt-4 flex justify-between items-center">
            <div className="flex space-x-3">
              ...
              <label 
                ...
                className="h-7 w-7 text-blue-500 cursor-pointer hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200"
              >
                <PhotoIcon className="h-full w-full" />
              </label>
              <GifIcon className="h-7 w-7 text-blue-500 cursor-pointer hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200" />
              <ChartBarIcon className="h-7 w-7 text-blue-500 cursor-pointer hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200" />
              <FaceSmileIcon 
                className="h-7 w-7 text-blue-500 cursor-pointer hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200"
                onClick={() => setShowEmojiPicker(!showEmojiPicker)}
              />
              <CalendarIcon className="h-7 w-7 text-blue-500 cursor-pointer hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200" />
            </div>
            <button
              type="submit"
              className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-7 rounded-full text-lg shadow-md hover:shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed animate-pulse-on-hover"
